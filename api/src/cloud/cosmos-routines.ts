...
// Cosmos-backed port of routines.ts. Document storage (head/snapshots) moves to Cosmos;
// admission gates, quota, fillers and media stay on Blob (see hybrid-admission.ts) until
// library-management.ts/library-references.ts (the GC/reference-scanning subsystem) is
// also ported -- that scanner is hard-wired to Blob today and must be able to see every
// reference before it's safe to let documents live anywhere else.
export class CosmosRoutines extends CloudDocumentsCosmos<CloudRoutine, Routine, ReferenceClaims> {
  ...
  constructor(cosmosStore: CosmosStoreLike, blobStore: BlobStore, auth: CloudAuth, readonly media: CloudMedia) {
    super(cosmosStore, auth, 'routine', blobDocumentsAdmission(blobStore));
    this.fillers = new CloudFillers(blobStore, auth, media);
  }
